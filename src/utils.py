...
def make_experiment_dir_name(max_model_size, experiment_name=None):
    """Return name of directory for results of given settings (only depends
        on model size) and experiment name"""
...
class LanguageGenerator:
    ...
    def ignore_or_add(self, expr, model_size):
    ...
    def write_expressions(self):
        ...
        out_dir = Path(self.dest_dir) / make_experiment_dir_name(
...
def load_expressions_for(experiment_name, max_model_size):
    """Given name of experiment and max model size, loads highest possible number
    of expressions from results dir of that experiment"""
    experiment_dir = (
        Path(PROJECT_DIR)
